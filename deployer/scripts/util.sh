...
function wait_for_fluentd_to_catch_up() {
    ...
    if os::cmd::try_until_text "curl_es ${es_pod} /project.logging.*/_count -X POST -d '$qs' | get_count_from_json" 1 $(( timeout * second )); then
    ...
    if os::cmd::try_until_text "curl_es ${es_ops_pod} /.operations.*/_count -X POST -d '$qs' | get_count_from_json" 1 $(( timeout * second )); then
