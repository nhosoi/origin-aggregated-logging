    ...
    class Output < Base
      ...
      def try_flush
        ...
        begin
          ...
          log.warn "NYA0: got unrecoverable error.", error: e 
          if @secondary
            if using_secondary
          ...
          log.warn "NYA1: taking back chunk for errors.", chunk: dump_unique_id_hex(chunk.unique_id)
          if output.delayed_commit
            @dequeued_chunks_mutex.synchronize do
      ...
      def update_retry_state(chunk_id, using_secondary, error = nil)
        @retry_mutex.synchronize do
          log.warn "NYA2: update_retry_state - incrementing num_errors", num_errors: @num_errors
          @counters_monitor.synchronize{ @num_errors += 1 }
          ...
          unless @retry
            ...
            if error
              log.warn "NYA3: failed to flush the buffer.", retry_time: @retry.steps, next_retry_seconds: @retry.next_time, chunk: chunk_id_hex, num_errors: @num_errors, error: error
